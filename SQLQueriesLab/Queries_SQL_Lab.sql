...
alter TABLE bank.trans ADD column incoming Float GENERATED ALWAYS as (`type`) VIRTUAL NOT NULL;

select account_id,
replace(`type`, "VYDAJ", "OUTGOING") as outgoing_amount,
replace(`type`, "PRIJEM", "INCOMING") as incoming_amount,
floor(sum(amount)) as total_amount
from bank.trans
where account_id=396
group by `type`
...
select account_id,floor(sum(amount)) as incoming_amount, floor(sum(amount)) as outgoing_amount
from bank.trans
where account_id=396
group by `type`
having `type`="VYDAJ" 
